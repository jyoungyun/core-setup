...
platformList.each { platform ->
    ...
    if (os == 'Windows_NT') {
        ...
        if (architecture == 'arm') {
            buildCommand = "./cross/build-rootfs.sh arm && ./build.sh --configuation ${configuration} --docker ubuntu.14.04 --env-vars DISABLE_CROSSGEN=1,TARGETPLATFORM=arm,TARGETRID=ubuntu.14.04-arm,CROSS=1 --targets Default"
        }
        else {
